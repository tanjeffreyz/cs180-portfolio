...
_, height, width = c_img.shape
resize = torchvision.transforms.Resize((int(height * config['scale']), int(width * config['scale'])))
c_img = resize(c_img)
...
result = (1 - config['noise']) * c_img.clone().contiguous() \
         + config['noise'] * torch.rand(c_img.shape).to(device)
